            ...
            <Stack gap={8} justify='space-between' align='center'>
                <Stack gap={12} align='center' style={{opacity: completed ? 0.3 : 1}}>
                    ...
                    <Stack direction="column" gap={4}>
                        <Typography type="note" lineClamp={1}>{title}</Typography>
                        <Typography type="mini" lineClamp={1} color='disabled'>{MAP_TASK_TYPES[type]}</Typography>
                        <Stack gap={4} align='center'>
                            <Typography type="mini" color="warning">{`${completed ? '' : '+'}${reward}`}</Typography>
                            <img width={16} height={16} src="/loaf_coin.png" />
                        </Stack>
                    </Stack>
                </Stack>
                <Emodji emodjiName={completed ? emodjiTypes.EmodjiName.SUCCESS_CHECK : emodjiTypes.EmodjiName.ARROW_RIGHT} />
            </Stack>
        </Card>
    )
}
